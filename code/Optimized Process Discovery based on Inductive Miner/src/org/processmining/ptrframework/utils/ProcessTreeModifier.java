...
//    }
    ...
    public boolean identifyFlower(Node node) {//修改花结构
        if (!(node instanceof Block.Xor)) {
            return false;
        }

        // Check if the XOR block has a flower structure
        List<Node> children = ((Block.Xor) node).getChildren();
        return children.size() == 2 && children.get(0) instanceof Task.Automatic;
    }
