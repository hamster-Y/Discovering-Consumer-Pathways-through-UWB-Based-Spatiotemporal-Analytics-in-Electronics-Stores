                    ...
                    if (leafLabels.contains(variant.get(variant.size() - 1))) {
                        ...
                        for (int j = 0; j < leavesReversed.size(); j++) {
                            if (i == j) {
                            	...
                            	// 将日志移动 (a, ">>") 修改为同步移动 (a, a)
//                                alignment.add(new Pair<>(variant.get(variant.size() - 1), variant.get(variant.size() - 1)));
...
//                        }
                    	// 如果未找到匹配项，将日志移动 (a, ">>") 修改为同步移动 (a, a)
                        alignment.add(new Pair<>(variant.get(parentState.index), variant.get(parentState.index)));
                      
                        for (ReplayProcessTree leaf : leavesReversed) {
                            alignment.add(new Pair<>(">>", leaf));
                            // 将剩余的日志移动 (">>", leaf) 修改为同步移动 (leaf.getLabel(), leaf)
//                            alignment.add(new Pair<>(leaf.getLabel(), leaf));
                    ...
                    for (ReplayProcessTree n : leavesReversed) {
                        ...
                        // 将日志移动 (">>", n) 修改为同步移动 (n.getLabel(), n)
              
//                        alignment.add(new Pair<>(n.getLabel(), n));
                    }
                }
            } else {
//                alignment.add(new Pair<>(variant.get(parentState.index), ">>"));
...
//        System.out.println("四大金刚摸排擀面皮美国经济高评级7"+result.getAlignment());
        return result;
    }
